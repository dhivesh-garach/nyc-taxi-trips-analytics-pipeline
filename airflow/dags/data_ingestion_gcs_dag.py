...
def format_to_parquet(src_file):
    ...
    pq.write_table(table, f'{path_to_local_home}/{parquet_file}')
...
with DAG(
    ...
    download_and_unzip_dataset_task = BashOperator(
        task_id="download_and_unzip_dataset_task",
        bash_command= f"""
        curl -sSL {dataset_url} > {path_to_local_home}/{dataset_file} && 
        if [ -f {path_to_local_home}/{csv_file} ]; then
        echo "File {csv_file} already exists. Removing it."
        rm {path_to_local_home}/{csv_file}
        fi && 
        gunzip {path_to_local_home}/{dataset_file}
        """
    )
    ...
    bigquery_external_table_task = BigQueryCreateExternalTableOperator(
        ...
        table_resource={
            "tableReference": {
                ...
                "tableId": "yellow_taxi_external_table",
            },
    ...
    download_and_unzip_dataset_task >> format_to_parquet_task >> local_to_gcs_task >> bigquery_external_table_task